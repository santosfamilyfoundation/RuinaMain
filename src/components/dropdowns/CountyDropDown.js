...
const CountyDropDown = (props) => {
    console.log("in county dropdown");
   ...
   let existingDataState = existingData
      ? existingData[stateDropDownQuestionID]
      : null;
         ...
         if (existingData[currId] != null) {
            ...
            if (existingDataState != null) {
               ...
               if (countiesByStates[existingDataState]) {
                  const correspondingCounties =
                     countiesByStates[existingDataState];
      ...
      // Check if user has updated state selection
         ...
         if (existingDataState != null) {
            ...
            if (!correspondingCounties.includes(selectedCounty)) {
               ...
               console.log("deleted county from state");
            }
         } else {
            // If user tries to select "Please select a state..." option in County dropdown without selecting a state
            setSelectedOption([]);
            setDeleteCountyFromState(true);
            console.log("set seleted option to null and deleted county from state");
         }
      }
   }
   ...
   useEffect(() => {
      if (deleteCountyFromState) {
         ...
         console.log("deleted road single response; deleted county from state");
      }
   });
   ...
   if (!existingData) {
      ...
      // if state question is not filled out
         ...
         if (countiesByStates[existingDataState]) {
            if (
               countiesByStates[existingDataState].includes(
                  existingData[currId]
               )
            ) {
               status = "success";
   ...
   if (existingDataState) {
      if (countiesByStates[existingDataState]) {
         countyOptions = countiesByStates[existingDataState]
            .sort()
            .map((county) => {
               return { name: county };
            });
   ...
   const validateLocal = (localInfo) => {
      ...
      if (localStatus) {
         ...
         setIsInvalid(false);
         console.log("set validation style to false");
      } else {
         setIsInvalid(true);
         setBorderColor("error.500");
         console.log("select validation style to true");
         return;
      }
   };
   ...
   const submitField = (selection) => {
      ...
      updateResponse &&
         updateResponse({ id, question: currId, selection: selected });
            ...
            <SectionedMultiSelect
               ...
               onSelectedItemObjectsChange={(selectedObject) =>
                  submitFunction({
                     id,
                     question: currId,
                     selection: selectedObject[0].name,
                  })
               }
