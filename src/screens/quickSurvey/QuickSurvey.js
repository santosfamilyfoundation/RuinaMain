/*
  ...
  relevant sections in the other screens. The QuickSurvey component receives a
  file path (for an in-progress report) or an empty string (for a new report)
  and loads the questions from the downloaded spreadsheet. If the component
  receives a file path, it fetches the saved JSON from the app file directory.
  When the JSON successfully loads with data, a dialog will alert the user and
  on acknowledgement will navigate to the Home screen. If the JSON fails or no
  file path is provided, the component renders the set up questions (the number
  of vehicles, non-motorists, type of property, if there was a secondary crash,
  and crash severity). The bottom of the page is a "Continue" button that
  navigates to the Home screen. The questions do not need to be answered to
...
class QuickSurvey extends Component {
	constructor(props) {
		...
		this.state = {
			autoSavedSession:
				this.props.navigation.getParam("autoSavedSession"), // should we try to load an autoSaved Report
		...
		await this.stateManager.RNFS.readFile(this.stateManager.path, "utf8")
			.then((data) => {
				try {
					...
					this.setState({
						loadedAutoSaveSuccessMessageVisible: true,
					});
				} catch (e) {
  ...
  */
	parseLoadedState(loadedState) {
		...
		this.props.addRoad({
			setupData: loadedQuiz["setupData"],
			roadID: roadID,
		});
				...
				return (
					<SafeAreaView style={{ flex: 1 }}>
						<MaterialDialog
							...
							visible={
								this.state.loadedAutoSaveSuccessMessageVisible
							}
							cancelLabel=""
							okLabel="Continue"
							onCancel={() => {
						...
						>
							<Text style={material.subheading}>
								Your last session is successfully loaded. Press
								Continue to edit the report.
							</Text>
						</MaterialDialog>
					</SafeAreaView>
				);
